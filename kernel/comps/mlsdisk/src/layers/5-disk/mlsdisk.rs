    ...
    fn enqueue(
        ...
        let handle_read_bio = |mut buf: Buf| {
            ...
            bio.segments().iter().for_each(|seg| {
                ...
                error_result!(seg.write_bytes(0, &buf.as_slice()[base..base + offset]));
                base += offset;
            });
            BioStatus::Complete
        };
        ...
        let handle_write_bio = |mut buf: Buf| {
            ...
            bio.segments().iter().for_each(|seg| {
                let offset = seg.nbytes();
                error_result!(seg.read_bytes(0, &mut buf.as_mut_slice()[base..base + offset]));
                base += offset;
            });
