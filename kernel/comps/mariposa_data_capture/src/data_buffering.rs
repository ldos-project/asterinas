...
impl ChunkingWriteWrapper {
    ...
    pub fn flush(&mut self) -> Result<usize, Box<dyn Error + 'static>> {
        ...
        // Wait for the write to complete before returning so that the VirtIO virtqueue
        // never fills up. If we fire-and-forget, flush_all's tight loop submits faster
        // than the device completes, the virtqueue runs out of descriptors, and
        // device.write() spin-waits with IRQs disabled — deadlocking the IRQ handler.
        let waiter = self
            .block_device
