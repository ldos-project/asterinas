...
use aster_block::{
    BlockDeviceMeta,
    bio::{
        Bio, BioEnqueueError, BioStatus, BioType, BioWaiter, BlockDeviceCompletionTrace,
...
impl BlockDevice {
    ...
    pub(crate) fn init(transport: Box<dyn VirtioTransport>) -> Result<(), VirtioDeviceError> {
        ...
        spawn_thread(server_for_oqueue.clone(), {
            let consumer = server_for_oqueue
...
impl aster_block::BlockDevice for BlockDevice {
    fn enqueue(&self, bio: SubmittedBio) -> Result<(), BioEnqueueError> {
        let reply_handle: Box<dyn Producer<BlockDeviceCompletionTrace>> =
