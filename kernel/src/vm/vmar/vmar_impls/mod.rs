...
use super::{
    PageFaultOQueueMessage, VMAR_CAP_ADDR, VMAR_LOWEST_ADDR,
    huge_pages::{VmMappingPolicyGreedyHugeMapping, huge_mapping_enabled},
...
pub struct Vmar {
    ...
    #[cfg(not(baseline_asterinas))]
    page_fault_oqueue_producer: ElementOQueueRef<oqueues::ObservableEvent<PageFaultOQueueMessage>>,
}
...
impl Vmar {
    ...
    fn add_rss_counter(&self, rss_type: RssType, val: isize) {
        ...
        let rss = self.rss_counters[rss_type as usize].add_on_cpu(cpu_id, val);
        self.rss_hwm_counters[rss_type as usize]
            .get_on_cpu(cpu_id)
...
#[repr(u32)]
#[derive(Clone, Copy, Debug, TryFromInt, PartialEq, Eq)]
pub enum RssType {
    File = 0,
    Anon = 1,
}
