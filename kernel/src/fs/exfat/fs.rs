...
use crate::{
    fs::{
        ...
        server_traits::{self, PageHandle, PageIOObservable as _, PageStore},
        utils::{CachePage, FileSystem, FsFlags, Inode, PageCache, SuperBlock},
    },
    prelude::*,
};
...
pub struct ExfatFS {
    ...
    ///
    /// TODO(arthurp): DON'T DO THIS, OMG. LET'S NEVER HAVE AD-HOC LOCKING!!!
    mutex: Mutex<()>,
}
...
impl PageStore for ExfatFS {
    fn read_page_async(&self, req: server_traits::AsyncReadRequest) -> Result<()> {
        ...
        let waiter = self.block_device.read_blocks_async_with_callback(
    ...
    fn write_page_async(&self, req: server_traits::AsyncWriteRequest) -> Result<()> {
        ...
        let waiter = self.block_device.write_blocks_async_with_callback(
            BlockId::new(req.handle.idx as u64),
            bio_segment,
            move |b| {
