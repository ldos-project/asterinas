...
use ostd::{
    ...
    sync::Waiter,
    tables::{
        registry::get_global_table_registry, Consumer, Producer, StrongObserver, Table,
        WeakObserver,
    },
};
...
use crate::{
    fs::utils::page_prefetch_policy::{new_waiter, PageAccessEvent},
    sched::{RealTimePolicy, SchedPolicy},
    thread::kernel_thread::ThreadOptions,
    time::{clocks::BootTimeClock, Clock},
};
...
pub fn start_regression_model_prefetch_policy() -> core::result::Result<(), Box<dyn core::error::Error>> {
    let registry = get_global_table_registry();
    let prefetcher_registration_table = registry
        .lookup::<PageCacheRegistrationCommand>(&path!(pagecache.policy.registration))
        .ok_or_else(|| ostd::tables::TableAttachError::Whatever {
    ...
    ThreadOptions::new(move || model_policy_thread_fn(prefetcher_registration_consumer, regression_model))
        .sched_policy(SchedPolicy::RealTime {
            rt_prio: 1.try_into().unwrap(),
            rt_policy: RealTimePolicy::default(),
        })
...
pub fn start_nn_model_prefetch_policy() -> core::result::Result<(), Box<dyn core::error::Error>> {
    let registry = get_global_table_registry();
    let prefetcher_registration_table = registry
        .lookup::<PageCacheRegistrationCommand>(&path!(pagecache.policy.registration))
        .ok_or_else(|| ostd::tables::TableAttachError::Whatever {
    ...
    ThreadOptions::new(move || model_policy_thread_fn(prefetcher_registration_consumer, nn_model))
        .sched_policy(SchedPolicy::RealTime {
            rt_prio: 1.try_into().unwrap(),
            rt_policy: RealTimePolicy::default(),
        })
...
/// The body of the prefetch policy thread. This is spawned in [`start_prefetch_policy`].
fn model_policy_thread_fn(
    prefetcher_registration_consumer: Box<dyn Consumer<PageCacheRegistrationCommand>>,
    model: impl Fn([i64; 2]) -> i64
) {
    info!("Reactive model prefetch policy started");
    ...
    loop {
        ...
        match wake_index {
            0 => {
                let mut register = || {
                    ...
                    if let Some(reg) = prefetcher_registration_consumer.try_take() {
                        println!("reactive_policy_thread_fn: registered new page cache (discarding the old one)");
                        access_waiter.waker().wake_up();
                        access_observers = Some(reg.access_table.attach_strong_observer()?);
                        prefetch_command_producer =
                            Some(reg.prefetch_command_table.attach_producer()?);
                    }
                    Ok::<_, Box<dyn core::error::Error>>(())
            ...
            1 => {
                if let Some(strong_observer) = &access_observers {
                    strong_observer.enqueue_for_strong_observe(access_waiter.waker());
                    if let Some(PageAccessEvent {
                        thread: Some(thread),
                        page,
                        ..
                    }) = strong_observer.try_strong_observe()
                    {
                        // POLICY
                        let features = [thread as i64, page as i64];
                        // let ffeatures = features.map(|v| v as f32);
                        let prediction = nn_model(features);
                        let stride = prediction - (page as i64);
                        let prediction = page + (stride as usize) * 8;
                        if let Some(prefetch_command_producer) = &prefetch_command_producer {
                            let put_res = prefetch_command_producer
                                .try_put(PrefetchCommand { page: prediction });
                            if put_res.is_some() {
                                info!(
                                    "Prefetching {prediction} for thread {thread} failed to send"
                                );
                            }
                        }
                    }
...
fn nn_model(arr: [i64; 2]) -> i64 {
    // fn nn_model(inputs: [f32; 2]) -> f32 {
    let inputs = arr.map(|v| v as f32);
    let weights: [f32; 2] = [0.2865, 1.0];
    let bias: f32 = -0.2013;
    return (inputs[0] * weights[0] + inputs[1] * weights[1] + bias) as i64;
}

fn regression_model(arr: [i64; 2]) -> i64 {
    assert!(arr.len() >= 2, "Input must be at least 2");
