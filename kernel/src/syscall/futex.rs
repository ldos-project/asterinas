...
use crate::{
    ...
    process::posix_thread::futex::{
        FutexFlags, FutexOp, futex_lock_pi, futex_op_and_flags_from_u32, futex_requeue,
...
pub fn sys_futex(
    ...
    let res = match futex_op {
        ...
        FutexOp::FUTEX_LOCK_PI => {
            let timeout = get_futex_timeout(utime_addr)?;
            futex_lock_pi(futex_addr, timeout, ctx, pid, false).map(|_| 0)
        }
        ...
        FutexOp::FUTEX_UNLOCK_PI => futex_unlock_pi(futex_addr, 1, ctx, pid),
        _ => {
