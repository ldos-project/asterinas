...
pub(crate) fn test_produce_consume(queue: CommunicationOQueueRef<TestMessage>) {
...
pub(crate) fn test_produce_strong_observe(queue: CommunicationOQueueRef<TestMessage>) {
    let producer = queue.attach_communication_producer().unwrap();
    let consumer = queue.attach_consumer().unwrap();
    let test_message = TestMessage { x: 42 };
...
pub(crate) fn test_produce_weak_observe(queue: CommunicationOQueueRef<TestMessage>) {
    let producer = queue.attach_communication_producer().unwrap();
    let consumer = queue.attach_consumer().unwrap();
...
pub(crate) fn test_send_receive_blocker(
    queue: CommunicationOQueueRef<TestMessage>,
    n_messages: usize,
    n_observers: usize,
) {
...
pub(crate) fn test_produce_strong_observe_only(queue: CommunicationOQueueRef<TestMessage>) {
    let test_message = TestMessage { x: 42 };
...
pub(crate) fn test_consumer_late_attach(queue: CommunicationOQueueRef<TestMessage>) {
    let producer = queue.attach_communication_producer().unwrap();
...
pub(crate) fn test_consumer_detach(queue: CommunicationOQueueRef<TestMessage>) {
    let producer = queue.attach_communication_producer().unwrap();
    let consumer = queue.attach_consumer().unwrap();
...
pub(crate) fn test_strong_observer_late_attach(queue: CommunicationOQueueRef<TestMessage>) {
    let producer = queue.attach_communication_producer().unwrap();
...
pub(crate) fn test_strong_observer_detach(queue: CommunicationOQueueRef<TestMessage>) {
    let producer = queue.attach_communication_producer().unwrap();
    let observer = queue
        .attach_strong_observer(ObservationQuery::new(|m: &TestMessage| m.x))
        .unwrap();
