    ...
    SPSCOQueue<T, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
    ...
    fn can_consume(&self) -> Option<usize> {
        self.can_consume_for_head(&self.head_index)
    }
    ...
    fn can_consume_for_head(&self, head: &AtomicUsize) -> Option<usize> {
    ...
    where
        T: Copy + Send,
    {
        let current_head = self.can_consume_for_head(head)?;
        let current_head_slot = self.mod_len(current_head);
...
{
    fn should_try(&self) -> bool {
        self.oqueue.can_consume().is_some()
    }
