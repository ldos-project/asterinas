...
/// require type-erased unsafe operations.
...
impl<T: Send + 'static> OQueueImplementation<T> {
    ...
    /// query results into the appropriate observation OQueues.
    ...
    /// Attach an observation producer to the OQueue if it is not a communication OQueue.
...
impl<T: 'static> ObservationRingBuffer<T> {
...
impl<T: Send> UntypedOQueueImplementation for OQueueImplementation<T> {
    fn detach_strong_observer(&self, observer_id: usize) {
        let mut inner = self.inner.lock();
