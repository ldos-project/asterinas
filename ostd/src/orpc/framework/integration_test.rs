...
mod test {
    ...
    use crate::{
        assert_eq_eventually, assert_matches_eventually,
        orpc::{
    ...
    use crate::orpc::framework::shutdown::{Shutdown, ShutdownState};

    const SLEEP_DURATION: Duration = Duration::from_millis(50);

    static SERVER_DROPS: AtomicUsize = AtomicUsize::new(0);

    fn wait_for_thread_exit(exited: &Arc<AtomicBool>) {
        for _ in 0..10 {
            if exited.load(Ordering::SeqCst) {
                return;
            }
            generic_test::sleep(SLEEP_DURATION);
        }
    }
    ...
    fn server_shutdown_and_cleanup() {
        ...
        wait_for_thread_exit(&server_ref.thread_exited);
        assert!(server_ref.thread_exited.load(Ordering::SeqCst));
    ...
    /// Test creating a new server instance with new OQueues after shutdown.
    #[ktest]
    fn server_restart_after_complete_teardown() {
        ...
        wait_for_thread_exit(&server1.thread_exited);
        assert!(server1.thread_exited.load(Ordering::SeqCst));
    ...
    /// Test reusing OQueues from a shutdown server for a new server instance.
    #[ktest]
