...
impl<S: HasVmReaderWriter + Send + Sync> VmIo for S {
    fn read(&self, offset: usize, writer: &mut VmWriter) -> Result<()> {
        ...
        let limit = offset.checked_add(writer.avail()).context(OverflowSnafu)?;
    ...
    fn read_bytes(&self, offset: usize, buf: &mut [u8]) -> Result<()> {
        ...
        let limit = offset.checked_add(buf.len()).context(OverflowSnafu)?;
        if limit > reader.remain() {
            return InvalidArgsSnafu.fail();
        }
    ...
    fn write(&self, offset: usize, reader: &mut VmReader) -> Result<()> {
        ...
        let limit = offset.checked_add(reader.remain()).context(OverflowSnafu)?;
...
impl<S: HasVmReaderWriter> VmIoFill for S {
    fn fill_zeros(&self, offset: usize, len: usize) -> core::result::Result<(), (Error, usize)> {
        ...
        if offset > writer.avail() {
            return Err((InvalidArgsSnafu.build(), 0));
        }
        ...
        if filled_len == len {
            ...
            Err((InvalidArgsSnafu.build(), filled_len))
        }
    }
}
