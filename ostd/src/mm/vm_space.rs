...
impl<'a> CursorMut<'a> {
    ...
    /// Run `f` on every mapped page in the range `start` to `end`. This will only run on non-I/O
    /// regions that have TLB entries.
        ...
        F: FnMut(&Range<Vaddr>, &Frame<dyn AnyUFrameMeta>, &PageProperty) -> Result<()>,
    {
        ...
        while self.virt_addr() < end {
            ...
            if let Some(VmQueriedItem::MappedRam {
                frame: ref sub_frame,
                prop: sub_props,
            }) = sub_mapping
            {
...
unsafe impl PageTableConfig for UserPtConfig {
    ...
    fn split_item(item: Self::Item) -> Self::Item {
        ...
        if let MappedItem::TrackedFrame(frame) = mapped_item {
    ...
    fn init_split_item_subpage(item: Self::Item, level: PagingLevel) {
        ...
        let MappedItem::TrackedFrame(frame) = mapped_item else {
            panic!("init_split_item_subpage called on I/O item")
        };
        ...
        let _ = ManuallyDrop::new(Frame::from_unused(frame.paddr(), (), level));
