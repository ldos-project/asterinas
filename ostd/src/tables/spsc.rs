...
// thread at a time.
unsafe impl<T, WQ: WaitMechanism, const STRONG_OBSERVERS: bool, const WEAK_OBSERVERS: bool> Sync
    for SpscTableCustom<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
}
unsafe impl<T, WQ: WaitMechanism, const STRONG_OBSERVERS: bool, const WEAK_OBSERVERS: bool> Send
    for SpscTableCustom<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
}

impl<T, WQ: WaitMechanism, const STRONG_OBSERVERS: bool, const WEAK_OBSERVERS: bool>
    SpscTableCustom<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
...
pub struct SpscProducer<
    ...
    const WEAK_OBSERVERS: bool,
> {
    table: Arc<SpscTableCustom<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>>,
...
static_assertions::assert_impl_any!(SpscProducer<(), WaitQueue, true, true>: Send);
static_assertions::assert_not_impl_any!(SpscProducer<(), WaitQueue, true, true>: Sync);

impl<T, WQ: WaitMechanism, const STRONG_OBSERVERS: bool, const WEAK_OBSERVERS: bool> Drop
    for SpscProducer<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
    fn drop(&mut self) {
...
impl<
        ...
        const STRONG_OBSERVERS: bool,
        const WEAK_OBSERVERS: bool,
    > Producer<T> for SpscProducer<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
    fn put(&self, data: T) {
        // let mut i = 0;
        let mut d = Some(data);
...
impl<T, WQ: WaitMechanism, const STRONG_OBSERVERS: bool, const WEAK_OBSERVERS: bool> Drop
    for SpscConsumer<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
    fn drop(&mut self) {
        let mut state = self.table.attachment_state.lock();
...
// Sanity checks because `PhantomData<Cell<()>>` does not overtly specify the expected auto-trait implementations.
static_assertions::assert_impl_any!(SpscStrongObserver<(), WaitQueue, true, true>: Send);
static_assertions::assert_not_impl_any!(SpscStrongObserver<(), WaitQueue, true, true>: Sync);
...
// Sanity checks because `PhantomData<Cell<()>>` does not overtly specify the expected auto-trait implementations.
static_assertions::assert_impl_any!(SpscWeakObserver<(), WaitQueue, true, true>: Send);
static_assertions::assert_not_impl_any!(SpscWeakObserver<(), WaitQueue, true, true>: Sync);

impl<T, WQ: WaitMechanism, const STRONG_OBSERVERS: bool, const WEAK_OBSERVERS: bool> Drop
    for SpscWeakObserver<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
    fn drop(&mut self) {
        let mut state = self.table.attachment_state.lock();
        let free_list = &mut state.free_weak_observer_bits;
...
impl<
        ...
        const STRONG_OBSERVERS: bool,
        const WEAK_OBSERVERS: bool,
    > WeakObserver<T> for SpscWeakObserver<T, WQ, STRONG_OBSERVERS, WEAK_OBSERVERS>
{
...
pub mod benchmark {
    ...
    use super::*;
    use crate::{
        benchmark::run_microbenchmark,
        sync::{WaitByYield, WaitByYieldThen},
        tables::generic_benchmarks,
    };
            ...
            for n_tables in [1, 4, 16, 64] {
                run_microbenchmark(
                    |iterations| {
                        ...
                        for _ in 0..n_tables {
                            tables.push(SpscTable::<usize>::new(size, 0, 0));
                        }
    ...
    pub fn benchmark_call_return_wait_types() {
        let size = 2;
        for n_pairs in [1, 4, 16, 64] {
                ...
                |iterations| {
                    let mut reporter =
                        generic_benchmarks::benchmark_call_return(iterations, n_pairs, || {
                            SpscTableCustom::<usize, WaitByYield>::new(size, 0, 0)
                        });
                    reporter.report("name", &"call_return_wait_types");
                    reporter.report("waiter", &"wait by yield");
                    reporter.report("queue_size", &size);
                    reporter
                },
                10000,
            ...
            run_microbenchmark(
                |iterations| {
                    let mut reporter =
                        generic_benchmarks::benchmark_call_return(iterations, n_pairs, || {
                            SpscTableCustom::<usize, WaitQueue>::new(size, 0, 0)
                        });
                    reporter.report("name", &"call_return_wait_types");
                    reporter.report("waiter", &"wait queue");
                    reporter.report("queue_size", &size);
                    reporter
                },
                10000,
            ...
            run_microbenchmark(
                |iterations| {
                    let mut reporter =
                        generic_benchmarks::benchmark_call_return(iterations, n_pairs, || {
                            SpscTableCustom::<usize, _>::new_with_waiters(
                                size,
                                0,
                                0,
                                WaitByYieldThen::new(2, Some(WaitQueue::new())),
                                WaitByYieldThen::new(2, Some(WaitQueue::new())),
                            )
                        });
                    reporter.report("name", &"call_return_wait_types");
                    reporter.report("waiter", &"wait by yield then wait queue");
                    reporter.report("queue_size", &size);
                    reporter
                },
                10000,
