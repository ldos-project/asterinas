...
expose events and state by creating OQueues (Observable Queues) and using ORPC (Observable Remote
Procedure Call) which automatically creates OQueues. Components can use those OQueues to observe
other components.
...
* **baseline_asterinas**: does not have ORPC or OQueues
...
* **Mariposa (default)**: has ORPC and OQueues
...
`#[cfg(not(baseline_asterinas))]` is used to gate Mariposa (ORPC based) code.
...
allowed (clippy runs with -A unused-imports there),
