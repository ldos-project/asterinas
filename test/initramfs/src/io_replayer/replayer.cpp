...
{
    if (argc < 7) {
        printf("Usage: ./replayer <baseline|failover> logfile <# of devices to trace (1,2,3)> <# of threads per device> /dev/tgt0-/dev/tgt1-/dev/tgt2 <n of devices traces>\n");
        exit(1);
    }
    ...
    int n_threads = std::stoi(std::string(argv[4]));
